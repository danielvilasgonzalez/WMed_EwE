...
compute_strata_fact_for_gsa <- function(gsa_num, gsa_sf_all, strata_def, resolution = 1) {
  ...
  ## per-cell area in km^2, accounting for latitude - grid cells are
  ## NOT equal-area (they shrink toward the poles). Cell-count alone was
  ## a reasonable approximation within a single GSA (narrow latitude
  ## range), but combining GSAs into a region-wide estimate needs true
  ## absolute area, not proportions derived from unequal-area cells.
  area_r <- raster::area(bathy_r)
  
  bathy_df <- as.data.frame(bathy_r, xy = TRUE)
  names(bathy_df)[3] <- "layer"
  area_df <- as.data.frame(area_r, xy = TRUE)
  names(area_df)[3] <- "cell_area_km2"
  ...
  ## both the proportion (within-GSA weighting, unchanged use) and the
  ## absolute area in km^2 (needed for the region-wide aggregate below)
  ## come from the same source data here, so they're always consistent
  strata_areas <- bathy_dt[, .(area_km2 = sum(cell_area_km2, na.rm = TRUE)), by = stratum_num]
  strata_areas[, prop := area_km2 / sum(area_km2)]
  strata_areas[, gsa := gsa_num]
  setorder(strata_areas, stratum_num)
  strata_areas[, .(gsa, stratum_num, area_km2, prop)]
}

## --- 5d. Run for every GSA in the study, cache to disk (bathymetry ) -------------------
## downloads are slow - don't repeat if already computed
strata_fact_cache_path <- file.path(out_dir, "./strata_fact_by_gsa.csv")

cache_valid <- file.exists(strata_fact_cache_path) &&
  "area_km2" %in% names(fread(strata_fact_cache_path, nrows = 1))
...
if (cache_valid) {
  ...
  if (file.exists(strata_fact_cache_path)) {
    message("\nCached strata_fact_by_gsa.csv is from before absolute-area was added",
            " (missing 'area_km2') - recomputing rather than using a stale cache.")
  }
...
## filtered by WMED GSA
n_hauls_by_stratum <- ta_dt[
  gsa %in% FILTER_AREAS & !is.na(stratum_num),
...
## get the mean over gsa year and stratum per FG (sum(dens)/n_hauls) = mean(dens)
...
## sum across strata -> the final GSA/year/FG density estimate (per-GSA)
...
## --- 5g. Region-wide (all GSAs combined) FG time series --------------------
## A simple mean of per-GSA densities would treat every GSA as equally
## representative of the Western Med regardless of its actual size - not
## correct, since GSAs differ substantially in total area. Instead,
## combine every (GSA, stratum) cell directly in one weighting stage
## using each cell's absolute area (km^2), avoiding any double-weighting
## from compounding two separate stages (strata-within-GSA, then
## GSA-within-region):
##   D_region = sum(D_gh * A_gh) / sum(A_gh)
## where D_gh is the per-haul mean density in GSA g's stratum h, and
## A_gh is that stratum's absolute area within that specific GSA.
demersal_fg_index_regional <- per_stratum_fg[
  ...
  .(mean_density_t_km2 = sum((density_sum / n_hauls) * area_km2, na.rm = TRUE) / sum(area_km2, na.rm = TRUE),
    n_gsa_contributing = uniqueN(gsa),
    n_hauls_total = sum(n_hauls)),
...
#apply filter for plotting purposes
...
plot_timeseries <- function(df, y, species_col, title, y_lab) {
  ggplot(df, aes(x = year, y = .data[[y]], color=as.factor(gsa))) + #, color = country
    ...
    theme(strip.text.y = element_text(angle = 0), legend.position = "bottom",axis.text.x = element_text(angle = 45,hjust=1))
}
...
#change strata naming without decimals
strata_profile[, stratum_label :=
                 gsub("([0-9]+)\\.([0-9]*)", "\\1", stratum_label)]

p_strata_profile <-
  ggplot(strata_profile[FG_name %in% top_fg_for_profile],
         aes(x = stratum_label,y = mean_density_per_haul, fill = stratum_num)) +
  geom_col() +
  scale_fill_gradient(low = "lightblue",high = "navy",guide = "none") +
  facet_wrap(vars(FG_name), scales = "free_y") +
  labs(title = "Depth-strata density profile by FG",x = "Depth stratum",y = "Mean density per haul (t/km²)") +
  theme_minimal(base_size = 10) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggsave(file.path(plot_dir, "demersal_fg_depth_strata_profile.png"), p_strata_profile,
       width = 16, height = 12, dpi = 150)

fwrite(strata_profile, file.path(out_dir, "demersal_fg_depth_strata_profile.csv"))
...
p6 <- plot_timeseries(a_fg_biom_data, "total_biomass_fg", "FG_name",
                      ...
                      "Total biomass")
ggsave(file.path(plot_dir, "acoustic_fg_biomass_timeseries.png"), p6, width = 14, height = 10, dpi = 150)
...
p7 <- plot_timeseries(a_fg_abund_data, "total_abundance_fg", "FG_name",
                      "Acoustic survey by FG and GSA",
                      "Total abundance")
ggsave(file.path(plot_dir, "acoustic_fg_abundance_timeseries.png"), p7, width = 14, height = 10, dpi = 150)

## SAVE INDECES ####
fwrite(demersal_fg_index, file.path(out_dir, "demersal_fg_annual_index.csv"))
fwrite(demersal_fg_index_regional, file.path(out_dir, "demersal_fg_annual_index_regional.csv"))
fwrite(acoustic_fg_index, file.path(out_dir, "acoustic_fg_annual_index.csv"))

message("\nDone. New FG-level outputs:")
message("- ", file.path(out_dir, "demersal_fg_annual_index.csv"), " (per-GSA, strata-weighted)")
message("- ", file.path(out_dir, "demersal_fg_annual_index_regional.csv"),
        " (all GSAs combined, area-weighted - the full Western Med FG time series)")
...
message("- ", file.path(out_dir, "/demersal_fg_depth_strata_profile.csv"), " (per-stratum density, for validation)")
